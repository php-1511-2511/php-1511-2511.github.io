...
[PHP 1511 Assignment Link](https://canvas.brown.edu/courses/1075279/assignments/7689048)

[PHP 2511 Assignment Link](https://canvas.brown.edu/courses/1075280/assignments/7689049)


***For the R Markdown Version of this assignment: [HW2.Rmd](../homework/hw2.Rmd)***
...
1. Your goal will be to build a model to predict birth weight. Begin by using number summaries and graphs to start to explore relationships of variables in this data set and `bwt`. (0.5point for summary table, 0.5point for all the plots)
```{r}
...
low.weight$race <- factor(low.weight$race,labels = c("white","black","other"))
low.weight$ui <- as.factor(low.weight$ui)
low.weight$smoke <- as.factor(low.weight$smoke)
low.weight$ht <- as.factor(low.weight$ht)
```
...
for (fit in fit.list) {
        ...
        ols_rvsp_plot(fit)
        ols_ovsp_plot(fit)
        ols_cooksd_barplot(fit)
}

```


**The variance of the model with $age$ seems not homogenous, the varaince of other models are homogenous. The models don't fit the data very well. There are outliers in all the models.**
...
```{r}
ols_rvsp_plot(fit10)
ols_rsd_qqplot(fit10)
ols_rsd_hist(fit10)
ols_norm_test(fit10)
ols_score_test(fit10)
```
...
```{r}
ols_cooksd_barplot(fit10)
ols_dfbetas_panel(fit10)
```
