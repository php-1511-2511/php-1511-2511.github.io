...
$$D_i= \dfrac{\sum_{j=1}^n \left( \hat{y}_j - \hat{y}_{j(i)}\right)^2}{pMSE}$$
...
## Marginal Model Plots with Outliers

```{r, eval=FALSE}
mod_cars2 <- lm(dist ~ speed, data=cars2)
mmps(mod_cars2)
```


## Marginal Model Plot Without Outliers

```{r, eval=FALSE}
mod_cars1 <- lm(dist ~ speed, data=cars1)
mmps(mod_cars1)
```
