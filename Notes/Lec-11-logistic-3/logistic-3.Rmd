...
Variable    Description
----------  ----------------------------------------            
pperf       Phsical Performance Scale (0-12)
...
For Self-Assessed Health Study we need to consider how we model certian variables

* Age: this is critcal since subjects were asked to compare their health to others
...
With our data we are mainly interested in how well self-assessed health predicts death. There are 11 other variables that are possible confounders for this relationship. This means that if we consider all of this we have the possibility of 12 main effects and 11 possible 2 way interactions with self assessed health. This could be a very large model and we are only thinking of 2-way interactions here. 

Due to these large possibilites we turn to automated procedures.

##Assessing Model Fit

Once we begin building models we need to know how to assess model fit.  We have previously covered the likelihood ratio test where we can compare one model versus another. Now we move to assessing the fit of one model.
...
The default is typically `G=10` to create deciles of increasing risk. Then we have
...
Recall our model chosen from our last selection technique:
```{r}
summary(mod.back.auto)
```
...
We can now see that we have `1 = Not Dead` and `2 = Dead`, we need this to be 0 and 1 respectively
```{R}
sah$dead <- sah$dead - 1
table(sah$dead)
```
...
This time we get get a p-value of `r hoslem.test(sah$dead, fitted(mod.back.auto), g=10)$p.value`. This means our model is a good fit and it is calibrated well. We could try other values of $G$ to make sure:
...
1. $\hat{p}_2$ the proability that subject 2 is dead.
...
* $p$ is arbitrary. 
* If we increase $p$ we increase the Specificity but descrease the Sensitvity. 
