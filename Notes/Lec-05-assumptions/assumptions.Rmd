...
library(ggplot2)
library(gridExtra)
p1 <- ggplot(anscombe, aes(x1,y1)) + geom_point() + geom_smooth(method="lm", se=FALSE)
p2 <- ggplot(anscombe, aes(x2,y2)) + geom_point() + geom_smooth(method="lm", se=FALSE)
p3 <- ggplot(anscombe, aes(x3,y3)) + geom_point() + geom_smooth(method="lm", se=FALSE)
p4 <- ggplot(anscombe, aes(x4,y4)) + geom_point() + geom_smooth(method="lm", se=FALSE)
...
```{R, eval=F}
library(ggplot2)
p1 <- ggplot(mod1, aes(.fitted, .resid)) + geom_point() 
p2 <- ggplot(mod2, aes(.fitted, .resid)) + geom_point()
p3 <- ggplot(mod3, aes(.fitted, .resid)) + geom_point()
p4 <- ggplot(mod4, aes(.fitted, .resid)) + geom_point()
grid.arrange(p1,p2,p3, p4, ncol=2)
...
ggplot(mod2a, aes(.fitted, .resid)) + geom_point()
```
...
- We will conside the framingham heart study data for this. 
...
- This shows that for those who are the same age a one unit increase in BMI leads to an  increase in systolic blood pressure of 1. 
...
\[\text{Standardized } \hat{\varepsilon}_i= \dfrac{\hat{\varepsilon}_i}{sd\left(\hat{\varepsilon}_i\right)}\]



## Why Standardize?

- This way we already assumed that $E\left(\varepsilon\right) = 0$ and the $var\left(\varepsilon\right) = \sigma^2$. 
- By standardizing we now have a mean of 0 still but this time a variance of 1. 
- This means that with the residuals being roughly normal we expect to see about 95\% of our residuals falling between $\pm2$. 
...
```{r, eval=F}
par(mfrow=c(1,2))
plot(anscombe$x1, resid(mod1), xlab="x1", ylab="Residuals")
plot(anscombe$x1, rstandard(mod1),xlab="x1", ylab=" Standardized Residuals")
```
